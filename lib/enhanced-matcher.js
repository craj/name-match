...
class EnhancedMatcher {
  ...
  getSimilarity(name1, name2) {
    ...
    const strategies = [() => this.exactMatchScore(normalized1, normalized2), () => this.tokenSetScore(normalized1, normalized2), () => this.initialsMatchScore(normalized1, normalized2), () => this.editDistanceScore(normalized1, normalized2)];
  ...
  normalizeNameForComparison(name) {
    ...
    const tokens = parsed.normalized.split(' ').filter(token => token.length > 0 && !this.stopwords.includes(token) && !this.prefixes.includes(token) && !this.suffixes.includes(token));
    return {
      original: name,
