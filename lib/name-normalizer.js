...
function cleanName(name) {
  if (!name) return '';
  return name.toLowerCase().replace(/[^\w\s'-]/g, ' ') // Replace special chars with space
...
function standardizeName(name) {
  ...
  const nameParts = [parsed.firstName, ...parsed.middleNames, parsed.lastName].filter(Boolean);
...
function getNameVariations(name) {
  ...
  for (const firstVariation of firstNameVariations) {
    variations.push([firstVariation, ...parsed.middleNames, parsed.lastName].filter(Boolean).join(' '));
  }

  // Add variations with middle initial
  if (parsed.middleNames.length > 0) {
    variations.push([parsed.firstName, parsed.initials.middle,
    // Just middle initials
    parsed.lastName].filter(Boolean).join(' '));

    // Also with first name variations
    for (const firstVariation of firstNameVariations) {
      if (firstVariation !== parsed.firstName) {
        variations.push([firstVariation, parsed.initials.middle, parsed.lastName].filter(Boolean).join(' '));
      }
    }
  }

  // Add variations with no middle name/initial
  variations.push([parsed.firstName, parsed.lastName].filter(Boolean).join(' '));

  // Also with first name variations
  for (const firstVariation of firstNameVariations) {
    if (firstVariation !== parsed.firstName) {
      variations.push([firstVariation, parsed.lastName].filter(Boolean).join(' '));
    }
  }
